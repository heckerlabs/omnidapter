...
```bash
uv run uvicorn omnidapter_server.main:app \
  --host 0.0.0.0 \
  --port 8000 \
  --workers 4 \
  --proxy-headers \
...
CMD ["uv", "run", "uvicorn", "omnidapter_server.main:app", \
...
server {
    ...
    location / {
        proxy_pass http://omnidapter_server;
...
| Metric | Description |
|---|---|
| `omnidapter_server_requests_total` | Request count by endpoint and status |
| `omnidapter_server_request_duration_ms` | p50/p95/p99 latency |
