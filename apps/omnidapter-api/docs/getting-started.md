...
`apps/omnidapter-api` (the hosted API) into the shared virtual environment.
...
```bash
cd apps/omnidapter-api
uv run uvicorn omnidapter_api.main:app --reload --host 0.0.0.0 --port 8000
```
