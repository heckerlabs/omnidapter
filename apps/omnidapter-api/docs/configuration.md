...
placed at `apps/omnidapter-api/.env`. Variable names are case-insensitive.

Settings are defined in `src/omnidapter_api/config.py` as a Pydantic
`BaseSettings` class.
...
2. `.env` file in `apps/omnidapter-api/`
