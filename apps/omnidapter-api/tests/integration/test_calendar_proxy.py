...
async def active_connection(session: AsyncSession, org: Organization) -> Connection:
    conn = Connection(
        id=uuid.uuid4(),
...
@pytest.mark.asyncio
async def test_usage_recorded_after_calendar_call(
    client: AsyncClient,
    session: AsyncSession,
    active_connection: Connection,
    org: Organization,
):
    """Usage record is written after a billable calendar call."""
