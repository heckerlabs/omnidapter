...
async def test_list_connections_isolation(
    ...
    owner_a = HostedConnectionOwner(
        id=uuid.uuid4(),
        tenant_id=(await db_session.get(HostedAPIKey, test_api_key[1].id)).tenant_id,
...
async def test_get_connection_isolation(
    ...
    response = await client.get(f"/v1/connections/{conn_a.id}", headers={"Authorization": f"Bearer {raw_key_a}"})
    assert response.status_code == 200

    # Fetch as Tenant B (404 Not Found)
    response = await client.get(f"/v1/connections/{conn_a.id}", headers={"Authorization": f"Bearer {raw_key_b}"})
...
async def test_delete_connection_isolation(
    ...
    response = await client.delete(f"/v1/connections/{conn_a.id}", headers={"Authorization": f"Bearer {raw_key_b}"})
    assert response.status_code == 404
    ...
    response = await client.delete(f"/v1/connections/{conn_a.id}", headers={"Authorization": f"Bearer {raw_key_a}"})
