...
async def test_provider_configs_isolation(
    ...
    response = await client.get("/v1/provider-configs", headers={"Authorization": f"Bearer {raw_key_a}"})
    ...
    response = await client.get("/v1/provider-configs", headers={"Authorization": f"Bearer {raw_key_b}"})
    assert response.status_code == 200
...
async def test_upsert_provider_config_isolation(
    ...
    raw_key_a, _ = test_api_key
    response = await client.get("/v1/provider-configs/google", headers={"Authorization": f"Bearer {raw_key_a}"})
    assert response.status_code == 200
