...
async def api_key_client(
    ...
    with unittest.mock.patch("omnidapter_hosted.dependencies.authenticate_hosted_key", AsyncMock(return_value=(api_key, test_tenant))):
...
async def test_list_calendars_proxy(
    mock_execute, 
...
@unittest.mock.patch("omnidapter_hosted.routers.calendar.execute_calendar_operation")
async def test_list_events_proxy(
    mock_execute, 
    api_key_client: AsyncClient
):
...
@unittest.mock.patch("omnidapter_hosted.routers.calendar.execute_calendar_operation")
async def test_create_event_proxy(
    mock_execute, 
    api_key_client: AsyncClient
):
    ...
    response = await api_key_client.post(
        f"/v1/connections/{conn_id}/calendars/{cal_id}/events", 
        json=payload
    )
...
@unittest.mock.patch("omnidapter_hosted.routers.calendar.execute_calendar_operation")
async def test_get_availability_proxy(
    mock_execute, 
    api_key_client: AsyncClient
):
