...
export function App() {
  ...
  if (isEmbedCallback) {
    useEffect(() => {
      ...
      if (cbConnectionId) {
        window.parent.postMessage({ type: "omnidapter:success", connectionId: cbConnectionId, provider: "" }, "*");
      } else if (cbStatus === "cancelled") {
        window.parent.postMessage({ type: "omnidapter:close" }, "*");
      } else {
        window.parent.postMessage({ type: "omnidapter:error", code: cbError, message: cbErrorDesc ?? "" }, "*");
      }
    }, []);
  ...
  useEffect(() => {
    ...
    const handler = (event: MessageEvent) => {
      ...
      if (data.type === "omnidapter:success") {
        addLog("success", "Connected via embed", `connection_id: ${data.connectionId}${data.provider ? `, provider: ${data.provider}` : ""}`);
        setEmbedSrc(null);
  ...
  const handleConnect = async () => {
    ...
    try {
      const redirectUri = mode !== "popup" ? `${window.location.origin}${window.location.pathname}` : undefined;
      ...
      if (mode === "popup") {
        ...
        sdkRef.current.open({
          ...
          onSuccess: ({ connectionId, provider }) =>
            addLog("success", "Connected via popup", `connection_id: ${connectionId}, provider: ${provider}`),
          onError: ({ code, message }) => addLog("error", `Error via popup: ${code}`, message),
