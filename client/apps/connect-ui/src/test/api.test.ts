...
describe("createSession", () => {
  it("returns the session token on success", async () => {
    vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
      new Response(JSON.stringify({ data: { session_token: "cs_abc123", expires_in: 900 } }), {
        status: 200,
  ...
  it("throws with code and message from FastAPI detail on failure", async () => {
    vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
      new Response(
        JSON.stringify({ detail: { code: "token_already_used", message: "Link already opened." } }),
  ...
  it("throws with code from detail.code for session_expired", async () => {
    vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
      new Response(
        JSON.stringify({ detail: { code: "session_expired", message: "Invalid or expired link token" } }),
        { status: 401, headers: { "Content-Type": "application/json" } }
      )
    );
...
describe("listProviders", () => {
  it("returns providers array on success", async () => {
    const providers = [{ key: "google", name: "Google", auth_kind: "oauth2", credential_schema: null }];
    vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
