...
function mockFetchSession(sessionToken = "cs_test_session_token_0123456789") {
    return vi.spyOn(globalThis, "fetch").mockImplementation(async (input) => {
        ...
        if (url.includes("/connect/session")) {
            return new Response(
                JSON.stringify({
                    data: { session_token: sessionToken, expires_in: 900 },
                    meta: {},
                }),
    ...
    it("SuccessView receives redirectUri from URL param after credential success", async () => {
        ...
        vi.spyOn(globalThis, "fetch").mockImplementation(async (input) => {
            ...
            if (url.includes("/connect/session")) {
                return new Response(
                    JSON.stringify({
                        data: { session_token: "cs_cred_session", expires_in: 900 },
                        meta: {},
                    }),
                    { status: 200, headers: { "Content-Type": "application/json" } }
                );
...
describe("bootstrap token exchange errors", () => {
    it("shows error view when token is already used", async () => {
            ...
            new Response(
                JSON.stringify({
                    detail: {
                        ...
                        message: "This link has already been opened.",
                    },
                }),
        ...
        await waitFor(() =>
            expect(
                screen.queryByText(/already been opened|session_error|api_error/i)
            ).not.toBeNull()
        );
    });
