...
export function App() {
    ...
    useEffect(() => {
        ...
        if (
            ...
            !errorCode &&
            window.opener &&
            localStorage.getItem("omnidapter_embed_oauth") === "true"
        ) {
                ...
                if (result.authorization_url) {
                    if (isInIframe()) {
                        ...
                        const left = Math.round(
                            Math.max(0, (screen.width - w) / 2 + (screenX ?? 0))
                        );
                        const top = Math.round(
                            Math.max(0, (screen.height - h) / 2 + (screenY ?? 0))
                        );
                        window.open(
    ...
    }, [state.view, state.selectedProvider, state.token, state.openerOrigin]);
