...
By default (`auto_register_by_env=True`), Omnidapter auto-registers OAuth providers only when their environment credentials are present. Apple auto-registration is opt-in via `OMNIDAPTER_APPLE_ENABLED=1`. You can always register providers manually (as shown below) to use constructor-based credentials.
