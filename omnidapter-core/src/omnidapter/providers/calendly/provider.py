"""Calendly provider registration class."""
...
from omnidapter.providers.calendly.metadata import CALENDLY_METADATA
from omnidapter.providers.calendly.oauth import CalendlyOAuthMixin


class CalendlyProvider(CalendlyOAuthMixin, BaseProvider):
    """Calendly provider."""
    ...
    def __init__(
        ...
        self._client_id = (
            client_id if client_id is not None else os.environ.get("CALENDLY_CLIENT_ID")
        )
        self._client_secret = (
            client_secret if client_secret is not None else os.environ.get("CALENDLY_CLIENT_SECRET")
        )

    @property
    def metadata(self) -> ProviderMetadata:
        return CALENDLY_METADATA
